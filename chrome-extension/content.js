/**
 * Tab Search — Chrome Content Script
 *
 * Injects a command-palette style overlay into every page. Activated by:
 *  1. Pressing Ctrl+P / Cmd+P on the page (keydown captured in capture phase
 *     so `preventDefault()` blocks the browser's Print dialog).
 *  2. A message from the background service worker when the user has manually
 *     assigned the "open-tab-search" command to Ctrl+P in
 *     chrome://extensions/shortcuts.
 *
 * The overlay is rendered inside a Shadow DOM so extension styles never
 * bleed into or get overridden by the host page's CSS.
 */

'use strict';

// Guard against double-injection (e.g. on history navigation)
if (!window.__tabSearchInjected) {
   ...
   * @param {string} query
   * @param {Array} tabs
   * @returns {Array<{tab,score,titleIndices,urlIndices}>}
   */
  ...
  /**
   * Wraps matched characters in <mark> tags for highlighting.
   * @param {string}   text
  ...
  /** @type {Array} All tabs fetched from background */
  let allTabs = [];
  /** @type {Array} Current filtered + scored results */
  let results = [];
  ...
  function buildOverlay() {
    ...
    // Host is invisible to layout — all sizing happens inside Shadow DOM
    host.style.cssText = [
    ...
    // Auto-focus with small delay to avoid page-level keydown handlers stealing focus
  ...
  function renderList() {
    ...
    list.innerHTML = visible.map((res, i) => {
      ...
      const urlShort   = truncate(tab.url || '', 90);
      const urlHtml    = highlightText(urlShort, urlIndices);
      const activeBadge = tab.active
        ? `<span class="ts-badge">current</span>`
        : '';

      return `
        <li
          class="ts-item${selected ? ' ts-item--active' : ''}"
          role="option"
  ...
  async function openOverlay() {
    ...
    let response;
    try {
      response = await chrome.runtime.sendMessage({ action: 'GET_TABS' });
    } catch (err) {
      // Extension context may be invalidated (e.g. after reload) — bail out silently
   ...
   * Runs before the page's own handlers, ensuring we can call
   * preventDefault() to suppress the browser's Print dialog.
   */
  document.addEventListener('keydown', (e) => {
    // Ctrl+P / Cmd+P — open Tab Search (and block Print dialog)
    ...
    // Allow Esc to close even when focus is outside the input
  ...
  // Message from background (commands API path)
  chrome.runtime.onMessage.addListener((msg) => {
    if (msg && msg.action === 'OPEN_TAB_SEARCH') openOverlay();
  });

  // =========================================================================
  // Styles (injected into Shadow DOM — never leaks to host page)
  // =========================================================================
